...
function longestSentence(text) {
  ...
  displaySummary(longestSentence);
}

function sortByWordCount(sentence1, sentence2) {
  let words1 = convertToWords(sentence1);
  let words2 = convertToWords(sentence2);

  return words1.length - words2.length;
}

function convertToWords(sentence) {
  let wordRegex = /[a-z-]+/ig;
  return sentence.match(wordRegex);
}

function displaySummary(sentence) {
  let words = convertToWords(sentence);
